...
class Database {
    ...
    public function createInternalUser($internalUser) {
        ...
        // Prepares the db-statement
        $statement = "INSERT INTO `t_internal_user` (`email`, `password`,`is_administrator`, `course_id`, `firstname`,`lastname`, `postal_code`,`city`, `street`) 
        VALUES ('$internalUser->email','$hashedPassword','$internalUser->isAdministrator''$internalUser->courseId','$internalUser->firstname','$internalUser->lastname',
    ...
    public function getAllInternalUser() {
        ...
        foreach ($sqlResult as $SqlInternalUser) {
            ...
            $internalUser->courseId = $SqlInternalUser['course_id'];
